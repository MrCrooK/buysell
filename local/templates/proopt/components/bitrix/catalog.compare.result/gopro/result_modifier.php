	...
	if(CModule::IncludeModule('redsign.grupper'))
	{
		...
		if(is_array($arResult['SHOW_PROPERTIES']) && count($arResult['SHOW_PROPERTIES'])>0)
		{
			...
			foreach($arResult['SHOW_PROPERTIES'] as $showProp){
				if(!in_array($showProp['CODE'], $arCodeUSED)){
					$arResult['SHOW_PROPERTIES'][$showProp['CODE']]['ALL_SAME'] = 'Y';
					$arResult['SHOW_PROPERTIES'][$showProp['CODE']]['ALL_EMPTY'] = 'Y';
					$arResult['NOT_GRUPED_PROPS']['CODES'][] = $showProp['CODE'];
					foreach($arResult['ITEMS'] as $iItemKey => $arItem){
						if(is_array($arItem['DISPLAY_PROPERTIES'][$showProp['CODE']]['VALUE']) && $arItem['DISPLAY_PROPERTIES'][$showProp['CODE']]['USER_TYPE'] != 'HTML'){
							$value = implode(' / ', $arItem['DISPLAY_PROPERTIES'][$showProp['CODE']]['DISPLAY_VALUE']);
						}
						else{
							$value = $arItem['DISPLAY_PROPERTIES'][$showProp['CODE']]['DISPLAY_VALUE'];
						}
						if($value!='')
							$arResult['SHOW_PROPERTIES'][$showProp['CODE']]['ALL_EMPTY'] = 'N';
						$value = ($value == '' ? '&nbsp;' : $value);
						$arResult['ITEMS'][$iItemKey]['DISPLAY_PROPERTIES'][$showProp['CODE']]['DISPLAY_PROPERTIES_FORMATED'] = $value;
						$arResult['ITEMS'][$iItemKey]['DISPLAY_PROPERTIES'][$showProp['CODE']]['DISPLAY_PROPERTIES_INTVAL'] = IntVal($value);
						if($iItemKey == 0)
							$last_value = $value;
						if($value != $last_value)
							$arResult['SHOW_PROPERTIES'][$showProp['CODE']]['ALL_SAME'] = 'N';
					}
					if($arResult['SHOW_PROPERTIES'][$showProp['CODE']]['ALL_EMPTY'] == 'N'){
